...
class TravelPlanInput(BaseModel):
    start_date: str = Field(
        description="The start date of the trip (YYYY-MM-DD) e.g. 2024-12-13."
    )
    end_date: str = Field(
        description="The end date of the trip (YYYY-MM-DD) e.g. 2024-12-19."
    )
...
Your mission is to create a detailed travel plan.
...
Use emojis to make your answers engaging and friendly.

{PydanticOutputParser(pydantic_object=TravelPlanOutput).get_format_instructions()}
"""
    ...
    output_parser = PydanticOutputParser(
        pydantic_object=TravelPlanOutput
    )
